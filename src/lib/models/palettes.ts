export const PALLETES: { [id: string]: string[] } = {
	//https://www.pixilart.com/palettes/aesthetic-emotions-46620
	AestheticEmotions: [
		'#F8B195',
		'#F67280',
		'#C06C84',
		'#6C5B7B',
		'#355C7D',
		'#99B898',
		'#FECEAB',
		'#FF847C',
		'#E84A5F',
		'#2A363B',
		'#A8A7A7',
		'#CC527A',
		'#E8175D',
		'#474747',
		'#363636',
		'#A8E6CE',
		'#DCEDC2',
		'#FFD3B5',
		'#FFAAA6',
		'#FF8C94',
		'#A7226E',
		'#EC2049',
		'#F26B38',
		'#F7DB4F',
		'#2F9599',
		'#E1F5C4',
		'#EDE574',
		'#F9D423',
		'#FC913A',
		'#FF4E50',
		'#E5FCC2',
		'#9DE0AD',
		'#45ADA8',
		'#547980',
		'#594F4F',
		'#FE4365',
		'#FC9D9A',
		'#F9CDAD',
		'#C8C8A9',
		'#83AF9B'
	],
	// https://www.pixilart.com/palettes/beachy-colors-ish-58225
	Beachy: [
		'#FFC98B',
		'#FFB284',
		'#F5CEC7',
		'#E79796',
		'#C6C09C',
		'#C73866',
		'#FE676E',
		'#FD8F58',
		'#FFBD71',
		'#FFDCA2',
		'#478BA2',
		'#DE5B6D',
		'#E9765B',
		'#F2A490',
		'#B9D4DB'
	],

	//https://www.pixilart.com/palettes/daydream20-37182
	DayDream: [
		'#430F43',
		'#472561',
		'#205973',
		'#248077',
		'#2D9A77',
		'#5EC688',
		'#AAE68F',
		'#64154D',
		'#8E184B',
		'#BA3155',
		'#D9505E',
		'#E3744F',
		'#F29E64',
		'#FFC477',
		'#FFDD96',
		'#FFF4B0',
		'#C22E35',
		'#D24F38',
		'#DF6939',
		'#ED9B4A'
	],
	// https://www.pixilart.com/palettes/neon-palette-31147
	Neon: [
		'#FF0000',
		'#FF1B00',
		'#FF4900',
		'#FF8000',
		'#FFB600',
		'#F6FF00',
		'#92FF00',
		'#52FF00',
		'#12FF00',
		'#00FF37',
		'#00FF89',
		'#00FFD1',
		'#00BFFF',
		'#0064FF',
		'#0024FF',
		'#6400FF',
		'#9B00FF',
		'#DB00FF',
		'#FF00C8',
		'#FF0092',
		'#FF0049'
	],
	// https://www.pixilart.com/palettes/retro-57703
	Retro: [
		'#845C51',
		'#6BBB9C',
		'#F6E0B2',
		'#EFAB64',
		'#DD7B62',
		'#BA555C',
		'#75A5AE',
		'#99C2A6',
		'#F5E9C4',
		'#E3B98F',
		'#C2977D',
		'#5D967F',
		'#E7A27A',
		'#E2816F',
		'#61465F'
	],
	// https://www.pixilart.com/palettes/royal-succulent-7687
	RoyalSuculent: [
		'#9C3E50',
		'#AB4250',
		'#CF5959',
		'#CF6563',
		'#E08484',
		'#DE9185',
		'#684F78',
		'#7F6187',
		'#8F6E8E',
		'#A16C9C',
		'#B074A8',
		'#5E4573',
		'#694A8C',
		'#6C549E',
		'#6D5DA6',
		'#706CB0',
		'#767EB8',
		'#465182',
		'#506494',
		'#5F7BA3',
		'#6691B0',
		'#37697A',
		'#377278',
		'#3B7C7D',
		'#498A86',
		'#5FA188',
		'#965D39',
		'#B3783D',
		'#B5913C',
		'#C4AD5A',
		'#D1C054',
		'#7C7BB5',
		'#7C83BF',
		'#8599C7',
		'#8DA2CC'
	],
	// https://www.pixilart.com/palettes/spooky-halloween-palette-37968
	SpookyHalloween: [
		'#420210',
		'#8F0517',
		'#B93A15',
		'#E97B21',
		'#FB9E3C',
		'#FCCD4F',
		'#CF8B1B',
		'#9A4C0B',
		'#802507',
		'#657D37',
		'#6A9520',
		'#71C026',
		'#72C92A',
		'#29074E',
		'#5C107D',
		'#9515B5',
		'#CC31EA',
		'#CBD9E0',
		'#A9C3CF',
		'#81919B',
		'#657A91',
		'#465478',
		'#3B2182',
		'#360F76',
		'#2A0963',
		'#2A2A4D',
		'#E9AFD4',
		'#D35EB4',
		'#982896',
		'#253614',
		'#373B0A',
		'#4C3E09',
		'#5B3F09',
		'#0B4831',
		'#063222',
		'#24503D'
	],
	// https://www.pixilart.com/palettes/good-pastelness-14050
	GoodPastelness: [
		'#C6F6DE',
		'#B7E4CE',
		'#A4C8B6',
		'#EBC3F6',
		'#CEABD8',
		'#B798C0',
		'#F2C4EA',
		'#D4AECD',
		'#C698BD',
		'#EEA9AE',
		'#D1979B',
		'#B17D80',
		'#F4ECBB',
		'#D5CEA2',
		'#F9EEA9',
		'#CAD2F3',
		'#A1ABD4',
		'#9DA5C3'
	],
	// https://www.pixilart.com/palettes/pastel-59795
	Pastel: [
		'#EBAB99',
		'#D95763',
		'#B34C5A',
		'#F3BC91',
		'#EB7D43',
		'#C16A40',
		'#FACC7E',
		'#FB9E27',
		'#CE8429',
		'#F5E487',
		'#E6C912',
		'#BDA619',
		'#D3E387',
		'#A7D03A',
		'#8BAB39',
		'#BDE898',
		'#75DB60',
		'#64B457',
		'#99E4E8',
		'#2AC9DE',
		'#29A6BA',
		'#AECAF8',
		'#639BFF',
		'#5681D5',
		'#C2BCE4',
		'#8983EC',
		'#746FC7',
		'#D2B3E6',
		'#AA6EDB',
		'#8E5EB8',
		'#EDB2E5',
		'#DD6CD9',
		'#B65CB7',
		'#F5B9D8',
		'#ED77B4',
		'#C36599',
		'#FFFFFF'
	],
	// https://www.pixilart.com/palettes/epic-32-11147  
	Epic32:["#453096","#4F65A8","#5C94D1","#87D4FA","#B8F2FF","#EBF5E7","#D6E6E1","#BED4D1","#A1B9BD","#858FAD","#614E85","#4F2269","#3D013D","#662545","#8A3A4E","#BA5D5D","#D67F71","#8F2B74","#D4618D","#F095AC","#FFBDBD","#FFE0DB","#D15069","#EB746C","#EB9E7B","#F0C590","#F0DFAD","#ECF0B4","#C0FAA3","#7AE69E","#5ECCA7","#51A6B0"]
};

export const INDEX_TO_PALLETE_MAP: { [id: number]: string[] } = Object.keys(PALLETES).reduce(
	(a, c, i) => ({ ...a, [i]: PALLETES[c] }),
	{}
);
export const PALLETE_KEYS = Object.keys(PALLETES);
